...
def task_population_method(policy, env, iteration):
    population_method(copy.deepcopy(policy), env, EPISODES_PM, ITERATIONS_PM, N_PM, iteration)
    save_path = f"{PATH_SCORES_POPULATION_METHOD}_{iteration}.txt"
    scores = load_list_from_file(save_path)
    plot_combined(scores, f"{PATH_PLOTS_POPULATION_METHOD}/combined/population_method_combined.png_{iteration}", EPISODES_PM)

def task_zeroth_order_method(policy, env, iteration):
    zeroth_order_method(copy.deepcopy(policy), env, LEARNING_RATE_ZOM, EPISODES_ZOM, ITERATIONS_ZOM, iteration)
    save_path = f"{PATH_SCORES_ZERO_ORDER_METHOD}_{iteration}.txt"
    scores = load_list_from_file(save_path)
    plot_combined(scores, f"{PATH_PLOTS_ZERO_ORDER_METHOD}/combined/zero_order_method_combined.png_{iteration}", EPISODES_ZOM)

def task_multithreaded():
    policy = p.Policy(STATE_DIM, ACTION_DIM, HIDDEN_DIM)
    env = gym.make('LunarLanderContinuous-v2')
    population_method_tasks = [(task_population_method, (policy, env, i)) for i in range(0, 12)]
    zeroth_order_method_tasks = [(task_zeroth_order_method, (policy, env, i)) for i in range(0, 12)]
    tasks = population_method_tasks + zeroth_order_method_tasks 
